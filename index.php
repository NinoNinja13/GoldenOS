...
?>
    ...
    <script>
        ...
        const allApps = <?= json_encode($all_apps, JSON_FORCE_OBJECT) ?>;
        ...
        function safeJSONParse(key, fallback = []) {
            try {
                ...
                if (item === null || item === "undefined" || item === "" || item === "null") return fallback;
                const parsed = JSON.parse(item);
                if (Array.isArray(fallback) && !Array.isArray(parsed)) return fallback;
                if (fallback !== null && typeof fallback === 'object' && !Array.isArray(fallback) && (typeof parsed !== 'object' || Array.isArray(parsed))) return fallback;
        ...
        let storedDexMode = localStorage.getItem('goldenos_dex_mode');
        // Default to Classic (false) if never set
        let isDexMode = storedDexMode === 'true';
        if (storedDexMode === null) {
            isDexMode = false;
        }
        let openDexWindows = [];
        ...
        function playModeSwitchAnimation(isInitialLoad = false) {
            ...
            setTimeout(() => {
                try {
                    ...
                    console.error("Startup Logic Error:", err);
                }

                // Always hide animation even if logic failed
                ...
                setTimeout(() => {
                    ...
                    try {
                        ...
                        console.error("Final UI display error:", err);
                    }
                }, 500);
            }, 2000);
        }
        ...
        function _startup() {
            ...
            setTimeout(() => {
                ...
                if (anim && anim.style.display !== 'none') {
                    console.warn("Safety trigger: Hiding stuck loader.");
                    anim.style.opacity = '0';
                    setTimeout(() => { 
                        anim.style.display = 'none';
                        const main = document.getElementById('main-content');
                        if (!isDexMode && main && main.style.display === 'none') {
            ...
            try {
                ...
                // Load All Settings (Individually safe)
                const loaders = [
                    loadDock, loadWallpaper, loadWidgetSettings, loadAccentColor,
                    loadAppTextColor, loadHideNamesSetting, loadRgbGamer,
                    loadDexTraySettings, loadCustomWebsClassic
                ];
                loaders.forEach(fn => {
                    try { if (typeof fn === 'function') fn(); } catch(e) { console.error("Error in loader:", e); }
                });
                ...
                if (showPhpAnimation) {
                    ...
                    } catch(e) { console.error("Error during mode init:", e); }
                }

                // Finalize UI
                try { _initAnimatedWallpaper(); } catch(e) {}
        ...
        function setAccentColor(color) {
            if (!color || typeof color !== 'string' || !color.startsWith('#')) color = '#FFD700';
